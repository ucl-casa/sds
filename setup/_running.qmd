...
```bash
podman run --rm -d --name {{< var env.name >}} -p 8888:8888 -p 4444:4444 \
   -v "$(pwd):/home/jovyan/work" \
  {{< var docker.amd >}} start.sh jupyter lab \
...
podman run --rm -d --name {{< var env.name >}} -p 8888:8888 -p 4444:4444 \
   -v "$(pwd):/home/jovyan/work" \
  {{< var docker.arm >}} start.sh jupyter lab \
  --NotebookApp.notebook_dir='/home/jovyan/work \
  --LabApp.password='' --ServerApp.password='' --NotebookApp.token=''
```
...
```bash
podman run --rm -d --name {{< var env.name >}} -p 8888:8888 -p 4444:4444 -v "$(pwd):/home/jovyan/work" {{< var docker.amd >}} start.sh jupyter lab --NotebookApp.notebook_dir='/home/jovyan/work --LabApp.password='' --ServerApp.password='' --NotebookApp.token=''
```

## Windows (ARM64)

```bash
podman run --rm -d --name {{< var env.name >}} -p 8888:8888 -v "$(pwd):/home/jovyan/work" {{< var docker.arm >}} start.sh jupyter lab --LabApp.password='' --ServerApp.password='' --NotebookApp.token=''
```

...
`sds-agent` image. It declares two providers — `ollama` and `openai` (both
...
Both provider blocks currently ship with empty `models` maps and no
top-level default `model` — there is no Ollama/OpenAI-compatible endpoint
configured yet. Populating them is exactly this skill's job, once
`OLLAMA_HOST`/`OPENAI_HOST` point at something real.
...
- Each key under `provider.ollama.models` / `provider.openai.models` is the
  ...
  `ollama/`/`openai/` in the top-level `model` field). The `name` field
  must repeat that same tag/id.
- `tool_call: true` marks the model as capable of tool use — every model
  in the file should set this unless there is a specific reason not to.
...
it was historically the first provider wired up.
...
Handles both an exact tag/id and a loose/partial name that needs resolving
...
`OLLAMA_HOST` points — this repo only talks to it over HTTP, it does not
run Ollama itself, so these commands are not run inside this repo's
...
2. Create the new tag from it:
   ...
   ```
   Name `<new-tag>` by appending the context size to the base tag, e.g.
   `some-model:8b` → `some-model:8b-128k`.
...
provider. `opencode.json` is authoritative for what actually ships — don't
let a model list drift into a second copy in a README or elsewhere.

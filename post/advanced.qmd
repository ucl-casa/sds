...
It's possible to have multiple virtual machines with different characteristics set up in Podman. So you might have a 'regular' machine for doing basic programing and a 'High-Performance' VM which is allowed to use many more cores and more system RAM to do intensive calculations or data processing much more quickly. Howver, if you'd like to do this it can all seem a little clunky because all of Podman's tools want to connect to the *default* machine. You can change this in the Podman Desktop, or on Mac/WSL in the Terminal by exporting an environment variable at the start of the session (or adding it to your RC file).

You can create an environment variable like so:
...
docker run --name {{< var env.name >}} --rm -d \
  ...
  {{< var docker.amd >}} jupyter lab --LabApp.password='' --ServerApp.password='' --NotebookApp.token=''
```
...
docker run --name {{< var env.name >}} --rm -d \
  -p 8888:8888 -p 4201:4201 -p 8787:8787 \
  -v "$(pwd):/home/jovyan/work" \
  {{< var docker.arm >}} jupyter lab --LabApp.password='' --ServerApp.password='' --NotebookApp.token=''
```
...
Remember to change the image name to {{< var docker.arm >}} if you are using a M-type Mac.
...
In this screenshot we're running one container named `fsds` that was built from the `jreades/sds:2022` image. Your container may have a different name (e.g. `{{< var env.name >}}`) but the *image* should be named `{{< var docker.amd >}}` or `{{< var docker.arm >}}`:
